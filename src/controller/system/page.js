...
module.exports = {
  ...
  async find (ctx) {
    const _ = ctx.query
    const rows = await knex.select().from('page').where(JSON.parse(_.whe)).limit(parseInt(_.lim) || 10).offset(parseInt(_.off) || 0)
    const count = await knex.count('* as cnt').from('page').where(JSON.parse(_.whe))
    ctx.body = ctx.succ({rows, count: count[0].cnt})
  },

  async findOne (ctx) {
    const idx = ctx.params
    const rows = await knex.first().from('page').where(idx)
  ...
  async update (ctx) {
    ...
    console.log(knex('page').where(idx).update(row).toString())
